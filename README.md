...
> **Fork of [SHAPY](https://github.com/muelea/shapy) adapted for CPU-only inference using Docker.**

This fork dockerizes the SHAPY body shape estimation model for CPU inference, removing all CUDA dependencies. It allows running body shape estimation from single RGB images **without requiring a GPU**.

## 🆕 What's New in This Fork

| Feature | Original | This Fork |
|---------|----------|-----------|
| **GPU Requirement** | CUDA required | CPU-only ✅ |
| **Installation** | Manual setup | Docker one-liner ✅ |
| **Keypoint Detection** | OpenPose (GPU) | MediaPipe (CPU) ✅ |
| **Rendering** | GPU OpenGL | OSMesa (CPU) ✅ |

### Key Changes

- **`Dockerfile`** - Complete Docker image with all dependencies
...
git clone https://github.com/YOUR_USERNAME/shapy-cpu.git
cd shapy-cpu

# 2. Download SHAPY_FILES from original repo and place in project root
# See original instructions: https://github.com/muelea/shapy
...
# 5. Run inference
docker-compose run shapy --image /app/input/your_image.jpg --gender neutral
```
...
```
output/
├── your_image.obj              # 3D body mesh
├── your_image_params.npz       # SMPL-X parameters (betas, pose)
└── your_image_measurements.json # Anthropometric measurements (height, weight, chest, waist, hips)
```

### Gender Options

```bash
docker-compose run shapy --image /app/input/photo.jpg --gender neutral  # Default
...
| Tested on | i5 11th gen, 20GB RAM |
